...
class SongsSubstate extends MusicBeatState{
	...
	/** this is literally freeplay but change so that only some songs are shown
		Current instance of `SongsSubstate`.
	**/
	public static var instance:SongsSubstate = null;
	...
	override function create() {
		...
		MusicBeatState.windowNameSuffix = " - " + CharterSelectorState.charterSelected + " Charts";
		...
		var initSonglist = CoolUtil.coolTextFile(Paths.txt("freeplaylist/" + CharterSelectorState.charterIconSelected + 'Songlist'));
		...
		DiscordClient.changePresence("Selecting a " + CharterSelectorState.charterSelected + " s", null);
		#end

		// Loops through all songs in da songlist
		...
		if (songsReady) {
			...
			if (FlxG.keys.justPressed.SPACE) {
				...
				if (Assets.exists(Paths.json("song data/" + songs[curSelected].songName.toLowerCase() + "/" + poop))) {
					PlayState.SONG = SongLoader.loadFromJson(poop, songs[curSelected].songName.toLowerCase());
